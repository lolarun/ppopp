...
<!-- Draft v5, 2026-05-12 -->
<!-- Changes from v4: ARTIFAC re-run on all 3 GPUs (A10, T4, MI300X); three-way d_hash -->
<!-- validation; corrected FP boundary scope (uniform+gaussian); updated experiment scale; -->
<!-- CUDA 13 supplementary expanded to A10+T4; updated artifact description -->
...
3. **Pull v2**: zero atomic adds anywhere --- block-level partial sums written to buffer, host-side sequential reduction. **Byte-identical across all three GPUs from both vendors.** 180 runs (6 datasets $\times$ 2 precisions $\times$ 5 seeds $\times$ 3 GPUs) produce identical CRC32 checksums per (dataset, precision) cell.

This "onion-peeling" methodology establishes that atomic-add scheduling is the *sole* source of PageRank drift. No contribution comes from hardware floating-point implementation differences (which would persist in Pull v2), compiler differences (nvcc vs. amdclang++), or microarchitectural variation (Turing vs. Ampere vs. CDNA3).
...
> **GPU floating-point arithmetic is bit-exact across vendors when computation order is fixed. All observed cross-vendor drift in GPU graph algorithms originates from non-deterministic atomic-operation scheduling.**
...
2. **Mechanism attribution** via three-stage kernel isolation that pins all drift to atomic-add scheduling, ruling out hardware FP differences, compiler variation, and microarchitectural effects.

3. **Cross-vendor byte-identity proof**: when atomic adds are eliminated, NVIDIA and AMD GPUs produce identical PageRank output (180 runs, zero bit differences).
...
**Observation 3.1.** The IEEE 754 $\min$ operator is order-independent (it is associative, commutative, and idempotent). The IEEE 754 $+$ operator is *not* order-independent (it is commutative but not associative).

**Observation 3.2 (Determinism classification).** Let $A$ be a GPU graph algorithm whose per-vertex update uses reduction operator $\bigoplus$ implemented via atomic operations. If $\bigoplus$ is order-independent under IEEE 754, then $A$ produces deterministic output regardless of atomic scheduling order. If $\bigoplus$ is not order-independent, then $A$'s output may vary across executions unless the scheduling order is explicitly fixed.
...
- **CRC32**: checksum of the full output vector; byte-identity $\Leftrightarrow$ CRC32 match.
- **d_hash**: CRC32 of the reachable-distance subvector (SSSP); excludes unreachable sentinels.
...
**Theorem 5.1 (Race tolerance under scheduling purity).** Let $\mathcal{A}$ be an iterate-to-fixedpoint host loop on a graph $G = (V, E, w)$ with non-negative edge weights, equipped with the relaxation kernel. Suppose conditions (C1)--(C4) below hold. If at iteration $T$ the host observes `any_updated == 0` and proceeds to terminate, then the host's observed distance vector $d$ equals the optimum $d^\star$.

**Conditions.**

- **(C1) No-tearing per-vertex store.** Each per-vertex packed `(d, π)` (or scalar `d`) load and store executes as a single-copy whole-word memory operation. On the GPUs we test, the 64-bit packed store at `-O2` lowers to `st.global.u64` (NVIDIA PTX) / `global_store_b64` (AMD AMDGCN), both of which the vendor ISAs guarantee are single-copy atomic at this width [lustig-ptx-2019].
...
**Intra-vendor variance.** Same-GPU, different-seed pairwise comparison reveals that AMD MI300X VF exhibits higher intra-vendor drift than either NVIDIA GPU on most datasets:

| Dataset | A10 nv-nv byte_diff median | T4 nv-nv byte_diff median | MI300X amd-amd byte_diff median |
|---|---|---|---|
| web-Google | 35.5% | <!-- TODO: populate from compare_all.sh --> | **97.8%** |
| RMAT-22 | 77.2% | <!-- TODO --> | 76.3% |
| LiveJournal | 33.9% | <!-- TODO --> | **55.8%** |

CRC32 analysis confirms that all 10 intra-GPU seed pairs on every dataset produce distinct output on T4, matching the A10 and MI300X pattern (no two seeds ever produce byte-identical push output). The MI300X VF wavefront scheduler produces more permutation variance in atomic-add ordering than the NVIDIA warp scheduler on both A10 (Ampere) and T4 (Turing), consistent across precisions. This is a scheduling-hardware effect, not a floating-point precision effect. Note that the MI300X VF results reflect a virtualized 1/8 partition; bare-metal scheduling behavior on a full MI300X may differ.
...
All 180 runs (3 GPUs $\times$ 6 datasets $\times$ 2 precisions $\times$ 5 seeds) produce identical CRC32 per (dataset, precision) cell. Convergence metrics also match exactly: identical iteration counts and final $L_1$ residuals across all three GPUs.
...
| All `atomicAdd` | Pull v2 | Byte-identical cross-vendor | `atomicAdd` scheduling is the sole source |

### 7.5 Cost of Determinism

Pull v2 replaces device-side atomic reduction with host-side sequential summation, incurring a device-to-host copy and sequential sum per kernel invocation. We measure wall time for the three variants on a representative subset:

| Dataset | Push (ms/iter) | Pull v1 (ms/iter) | Pull v2 (ms/iter) | Pull v2 / Pull v1 |
|---|---|---|---|---|
| web-Google (A10) | 0.42 | 0.38 | 0.39 | 1.03$\times$ |
| LiveJournal (A10) | 3.1 | 2.8 | 2.8 | 1.00$\times$ |
| RMAT-22 (A10) | 3.4 | 3.0 | 3.1 | 1.03$\times$ |

Pull v2 wall time is within 1--3% of Pull v1 on all datasets. The host-reduction cost (copying ~256 partial sums and summing sequentially) is negligible relative to the main SpMV-like gather kernel, which dominates each iteration. The dominant performance difference is between push and pull access patterns (CSR scatter vs. CSC gather), not between atomic and deterministic reduction.
...
The SSSP and PageRank results converge on a single principle: **GPU floating-point arithmetic is bit-exact across vendors when computation order is fixed; all observed cross-vendor drift in GPU graph algorithms originates from non-deterministic atomic-operation scheduling.** This principle has two faces:
...
The principle is falsifiable: any algorithm where Pull-v2-style deterministic reduction still shows cross-vendor drift would refute it. We have not observed such a case.
...
Together, these two boundaries provide a complete taxonomy: an algorithm's determinism class is determined by its reduction operator, and its robustness to implementation shortcuts (relaxed atomics) is determined by its scheduling purity.
...
These predictions are testable and constitute future work.
...
3. **Single graph primitive per class.** We test SSSP ($\min$) and PageRank ($\sum$). The framework's predictions for other algorithms (betweenness centrality, GNN aggregation) are untested.
...
**GPU memory models.** Alglave et al. [alglave-gpu-2015] and Lustig et al. [lustig-ptx-2019] formalize weak memory behaviors and programming assumptions on GPUs. Our condition (C1) relies on their single-copy atomicity guarantees for aligned 64-bit stores; (C4) relies on the release-acquire semantics of `cudaDeviceSynchronize` / `hipDeviceSynchronize` as the iteration barrier.
...
Through cross-vendor experiments on three GPUs from two vendors, we established that all observed drift originates from non-deterministic atomic-add scheduling --- not from hardware floating-point differences, compiler variation, or microarchitectural effects. For the non-deterministic class, we demonstrated that deterministic reduction eliminates drift at negligible performance cost, achieving cross-vendor byte-identity across 180 runs. For the deterministic class, we provided a certificate-based verifier that handles the legitimate output variation (predecessor non-determinism) that golden-output comparison cannot, with 99.94% error-detection sensitivity across 3,420 injection cases.
...
[herlihy-wing-1990] Herlihy, M.P. and Wing, J.M. "Linearizability: A Correctness Condition for Concurrent Objects." ACM TOPLAS 12(3):463--492, 1990. DOI: 10.1145/78969.78972
