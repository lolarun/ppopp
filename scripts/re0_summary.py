...
for f in sorted(glob.glob("results/_compare/*.json")):
    ...
    if pair == ("nvidia", "nvidia"):
        ...
        kind = "nv-amd"
    buckets[(d["dataset"], kind)].append(d)

hdr = ("dataset", "pair", "N", "byte_diff_med",
       "byte_diff_max", "max_Linf_max", "top100_J_min", "top100_K_min")
print(f"{hdr[0]:<14}{hdr[1]:<10}{hdr[2]:>4}{hdr[3]:>16}{hdr[4]:>16}{hdr[5]:>16}{hdr[6]:>14}{hdr[7]:>14}")
print("-" * 104)
for key in sorted(buckets, key=lambda k: (k[0], {"nv-nv": 0, "amd-amd": 1, "nv-amd": 2}[k[1]])):
    rows = buckets[key]
    ...
    ds, pk = key
    print(f"{ds:<14}{pk:<10}{len(rows):>4}{S.median(bd):>15.2%}"
          f"{max(bd):>15.2%}{max(mi):>16.3e}{min(jc):>14.3f}{min(ke):>14.3f}")

# Verdict
print()
nvamd = [r for k, rs in buckets.items() if k[1] == "nv-amd" for r in rs]
